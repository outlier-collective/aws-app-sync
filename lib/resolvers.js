...
const { isNil, equals, filter, find, map, not, flatten, merge, pipe, pick } = require('ramda')
...
const createOrUpdateResolvers = async (appSync, config, debug) => {
  ...
  console.log(deployedResolvers)

  const resolversWithTemplates = await Promise.all(
    map(async (template) => {
      let requestMappingTemplate = template.requestMappingTemplate
